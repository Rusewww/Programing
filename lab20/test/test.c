...
bool insertLinkTest()
{
	...
	for (; watch != NULL && tmp != NULL; watch = watch->next, tmp = tmp->next) {
		if (watch->waterproof == tmp->waterproof && tmp->cost == watch->cost && tmp->style == watch->style &&
		    strcmp(watch->model, tmp->model) == 0 && strcmp(tmp->manufacturer.firm, watch->manufacturer.firm) == 0) {
			result &= true;
		} else {
			result &= false;
		}
